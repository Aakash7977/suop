/**
 * AccountsPayableService — SupplierInvoice Service Layer
 *
...
export const AccountsPayableService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List supplier_invoices with pagination, status filter, and full-text search.
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single SupplierInvoice by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).SupplierInvoices.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('SupplierInvoice', id)
    }
    return record
  },
   ...
   *   1. INSERT into supplier_invoices
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=SupplierInvoiceCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    if (!data.invoice_number) {
      throw new ValidationError('SupplierInvoice.invoice_number is required', [
    ...
    if (existing) {
      throw new BusinessRuleError(
        `SupplierInvoice with invoice_number='${data.invoice_number}' already exists`,
        { code: 'SUPPLIERINVOICE_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).SupplierInvoices.create({
        data: {
          ...data,
          tenantId,
    ...
    logger.info('SupplierInvoice created', { id, invoice_number: data.invoice_number })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing SupplierInvoice.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).SupplierInvoices.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('SupplierInvoice', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `SupplierInvoice '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).SupplierInvoices.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
    ...
    logger.info('SupplierInvoice updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a SupplierInvoice.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).SupplierInvoices.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('SupplierInvoice soft-deleted', { id })
  },
   ...
   * (e.g. 'SupplierInvoiceLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'SupplierInvoiceLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'SupplierInvoice'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).SupplierInvoices.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('SupplierInvoice', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'SupplierInvoice' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await eventBus.writeToOutbox({
        eventName: 'SupplierInvoiceTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('SupplierInvoice transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count SupplierInvoice records matching optional status filter.
  ...
  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a SupplierInvoice with the given invoice_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(invoice_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).SupplierInvoices.findFirst({
