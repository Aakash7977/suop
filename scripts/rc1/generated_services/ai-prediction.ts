/**
 * AiPredictionService — AiPrediction Service Layer
 *
...
export const AiPredictionService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List ai_predictions with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { prediction_id: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).AiPredictions.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).AiPredictions.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single AiPrediction by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).AiPredictions.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('AiPrediction', id)
    }
    return record
  },
  ...
  // ═══ CREATE ════════════════════════════════════════════════════════════════
  /**
   * Create a new AiPrediction.
   *
   * Business rules:
   *   - prediction_id must be unique within tenant
   ...
   *   3. Write event to outbox (event=AiPredictionGenerated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    if (!data.prediction_id) {
      throw new ValidationError('AiPrediction.prediction_id is required', [
        { field: 'prediction_id', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).AiPredictions.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `AiPrediction with prediction_id='${data.prediction_id}' already exists`,
        { code: 'AIPREDICTION_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).AiPredictions.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.prediction_id),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'AiPredictionGenerated',
        payload: { id: created.id, tenantId, prediction_id: data.prediction_id },
        tenantId,
      })
    ...
    logger.info('AiPrediction created', { id, prediction_id: data.prediction_id })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing AiPrediction.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).AiPredictions.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('AiPrediction', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `AiPrediction '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).AiPredictions.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'AiPrediction',
        entityId: id,
        entityCode: String(before.prediction_id),
        before,
        after: updated,
      })
    ...
    logger.info('AiPrediction updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a AiPrediction.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).AiPredictions.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('AiPrediction soft-deleted', { id })
  },
   ...
   * (e.g. 'AiPredictionLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'AiPredictionLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'AiPrediction'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).AiPredictions.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('AiPrediction', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'AiPrediction' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'AiPrediction',
        entityId: id,
        entityCode: String(entity.prediction_id),
      ...
      await eventBus.writeToOutbox({
        eventName: 'AiPredictionTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('AiPrediction transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count AiPrediction records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).AiPredictions.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a AiPrediction with the given prediction_id exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(prediction_id: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).AiPredictions.findFirst({
      where: { tenantId, prediction_id, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
