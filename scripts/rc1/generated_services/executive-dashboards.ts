/**
 * ExecutiveDashboardsService — Dashboard Service Layer
 *
...
// ─── ExecutiveDashboardsService ─────────────────────────────────────────────────────────────

export const ExecutiveDashboardsService = {

  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List dashboards with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { dashboard_code: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).Dashboards.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).Dashboards.count({ where }),
    ])
  ...
  async getById(id: string): Promise<Record<string, unknown>> {
    ...
    const record = await (db as any).Dashboards.findFirst({
   ...
   *
   * Business rules:
   *   - dashboard_code must be unique within tenant
   *   - status defaults to 'DRAFT' (workflow initialState)
   *   - version initialized to 0
   *
   * Side effects (all in one transaction):
   *   1. INSERT into dashboards
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=DashboardCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    const { tenantId, userId, userEmail, correlationId } = getContext()

    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.dashboard_code) {
      throw new ValidationError('Dashboard.dashboard_code is required', [
        { field: 'dashboard_code', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).Dashboards.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `Dashboard with dashboard_code='${data.dashboard_code}' already exists`,
        { code: 'DASHBOARD_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).Dashboards.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.dashboard_code),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'DashboardCreated',
        payload: { id: created.id, tenantId, dashboard_code: data.dashboard_code },
        tenantId,
      })
    ...
    logger.info('Dashboard created', { id, dashboard_code: data.dashboard_code })
    return { id }
  },
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).Dashboards.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('Dashboard', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `Dashboard '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).Dashboards.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'Dashboard',
        entityId: id,
        entityCode: String(before.dashboard_code),
        before,
        after: updated,
      })
    ...
    logger.info('Dashboard updated', { id, version: result.version })
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).Dashboards.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('Dashboard soft-deleted', { id })
  },
   ...
   * (e.g. 'DashboardLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'DashboardLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'Dashboard'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).Dashboards.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('Dashboard', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'Dashboard' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'Dashboard',
        entityId: id,
        entityCode: String(entity.dashboard_code),
      ...
      await eventBus.writeToOutbox({
        eventName: 'DashboardTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
          to: targetState,
          dashboard_code: entity.dashboard_code,
        },
        tenantId,
      })
    ...
    logger.info('Dashboard transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count Dashboard records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).Dashboards.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a Dashboard with the given dashboard_code exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(dashboard_code: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).Dashboards.findFirst({
      where: { tenantId, dashboard_code, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
