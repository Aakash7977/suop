/**
 * BiFoundationService — BiKpi Service Layer
 *
...
export const BiFoundationService = {

  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List bi_kpi_repository with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { kpi_code: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).BiKpiRepository.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).BiKpiRepository.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single BiKpi by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).BiKpiRepository.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('BiKpi', id)
    }
    return record
  },
   ...
   *
   * Business rules:
   *   - kpi_code must be unique within tenant
   *   - status defaults to 'DRAFT' (workflow initialState)
   *   - version initialized to 0
   *
   * Side effects (all in one transaction):
   *   1. INSERT into bi_kpi_repository
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=KpiDefined)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.kpi_code) {
      throw new ValidationError('BiKpi.kpi_code is required', [
        { field: 'kpi_code', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).BiKpiRepository.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `BiKpi with kpi_code='${data.kpi_code}' already exists`,
        { code: 'BIKPI_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).BiKpiRepository.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityType: 'BiKpi',
        entityId: created.id,
        entityCode: String(data.kpi_code),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'KpiDefined',
        payload: { id: created.id, tenantId, kpi_code: data.kpi_code },
        tenantId,
      })
    ...
    logger.info('BiKpi created', { id, kpi_code: data.kpi_code })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing BiKpi.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).BiKpiRepository.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('BiKpi', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `BiKpi '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).BiKpiRepository.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'BiKpi',
        entityId: id,
        entityCode: String(before.kpi_code),
        before,
        after: updated,
      })
    ...
    logger.info('BiKpi updated', { id, version: result.version })
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).BiKpiRepository.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('BiKpi soft-deleted', { id })
  },
   ...
   * (e.g. 'BiKpiLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'BiKpiLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'BiKpi'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).BiKpiRepository.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('BiKpi', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'BiKpi' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'BiKpi',
        entityId: id,
        entityCode: String(entity.kpi_code),
      ...
      await eventBus.writeToOutbox({
        eventName: 'BiKpiTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
          to: targetState,
          kpi_code: entity.kpi_code,
        },
        tenantId,
      })
    ...
    logger.info('BiKpi transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count BiKpi records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).BiKpiRepository.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a BiKpi with the given kpi_code exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(kpi_code: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).BiKpiRepository.findFirst({
      where: { tenantId, kpi_code, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
