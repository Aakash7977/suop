/**
 * AccountsReceivableService — CustomerInvoice Service Layer
 *
...
export const AccountsReceivableService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List customer_invoices with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    const [rows, total] = await Promise.all([
      (db as any).CustomerInvoices.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).CustomerInvoices.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single CustomerInvoice by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).CustomerInvoices.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('CustomerInvoice', id)
    }
    return record
  },
  ...
  // ═══ CREATE ════════════════════════════════════════════════════════════════
  /**
   * Create a new CustomerInvoice.
   *
   * Business rules:
   *   - invoice_number must be unique within tenant
   *   - status defaults to 'DRAFT' (workflow initialState)
   *   - version initialized to 0
   *
   * Side effects (all in one transaction):
   *   1. INSERT into customer_invoices
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=CustomerInvoiceCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    if (!data.invoice_number) {
      throw new ValidationError('CustomerInvoice.invoice_number is required', [
        { field: 'invoice_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).CustomerInvoices.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `CustomerInvoice with invoice_number='${data.invoice_number}' already exists`,
        { code: 'CUSTOMERINVOICE_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).CustomerInvoices.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.invoice_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'CustomerInvoiceCreated',
        payload: { id: created.id, tenantId, invoice_number: data.invoice_number },
        tenantId,
      })
    ...
    logger.info('CustomerInvoice created', { id, invoice_number: data.invoice_number })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing CustomerInvoice.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).CustomerInvoices.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('CustomerInvoice', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `CustomerInvoice '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).CustomerInvoices.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'CustomerInvoice',
        entityId: id,
        entityCode: String(before.invoice_number),
        before,
        after: updated,
      })
    ...
    logger.info('CustomerInvoice updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a CustomerInvoice.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).CustomerInvoices.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('CustomerInvoice soft-deleted', { id })
  },
   ...
   * (e.g. 'CustomerInvoiceLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'CustomerInvoiceLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'CustomerInvoice'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).CustomerInvoices.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('CustomerInvoice', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'CustomerInvoice' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'CustomerInvoice',
        entityId: id,
        entityCode: String(entity.invoice_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'CustomerInvoiceTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('CustomerInvoice transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count CustomerInvoice records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).CustomerInvoices.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a CustomerInvoice with the given invoice_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(invoice_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).CustomerInvoices.findFirst({
      where: { tenantId, invoice_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
