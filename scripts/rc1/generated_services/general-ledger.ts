/**
 * GeneralLedgerService — JournalEntry Service Layer
 *
...
export const GeneralLedgerService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List journal_entries with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { entry_number: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).JournalEntries.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).JournalEntries.count({ where }),
    ])
  ...
  async getById(id: string): Promise<Record<string, unknown>> {
    ...
    const record = await (db as any).JournalEntries.findFirst({
   ...
   *
   * Business rules:
   *   - entry_number must be unique within tenant
   ...
   *   3. Write event to outbox (event=JournalEntryCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    if (!data.entry_number) {
      throw new ValidationError('JournalEntry.entry_number is required', [
        { field: 'entry_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).JournalEntries.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `JournalEntry with entry_number='${data.entry_number}' already exists`,
        { code: 'JOURNALENTRY_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).JournalEntries.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.entry_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'JournalEntryCreated',
        payload: { id: created.id, tenantId, entry_number: data.entry_number },
        tenantId,
      })
    ...
    logger.info('JournalEntry created', { id, entry_number: data.entry_number })
    return { id }
  },
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).JournalEntries.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('JournalEntry', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `JournalEntry '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).JournalEntries.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'JournalEntry',
        entityId: id,
        entityCode: String(before.entry_number),
        before,
        after: updated,
      })
    ...
    logger.info('JournalEntry updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a JournalEntry.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).JournalEntries.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('JournalEntry soft-deleted', { id })
  },
   ...
   * (e.g. 'JournalEntryLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'JournalEntryLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'JournalEntry'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).JournalEntries.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('JournalEntry', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'JournalEntry' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'JournalEntry',
        entityId: id,
        entityCode: String(entity.entry_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'JournalEntryTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('JournalEntry transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count JournalEntry records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).JournalEntries.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a JournalEntry with the given entry_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(entry_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).JournalEntries.findFirst({
      where: { tenantId, entry_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
