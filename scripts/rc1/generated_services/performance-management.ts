/**
 * PerformanceManagementService — PerformanceReview Service Layer
 *
...
export const PerformanceManagementService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List performance_reviews with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { review_number: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).PerformanceReviews.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).PerformanceReviews.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single PerformanceReview by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).PerformanceReviews.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('PerformanceReview', id)
    }
    return record
  },
   ...
   *
   * Business rules:
   *   - review_number must be unique within tenant
   *   - status defaults to 'DRAFT' (workflow initialState)
   *   - version initialized to 0
   *
   * Side effects (all in one transaction):
   *   1. INSERT into performance_reviews
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=PerformanceReviewInitiated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    const { tenantId, userId, userEmail, correlationId } = getContext()

    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.review_number) {
      throw new ValidationError('PerformanceReview.review_number is required', [
        { field: 'review_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).PerformanceReviews.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `PerformanceReview with review_number='${data.review_number}' already exists`,
        { code: 'PERFORMANCEREVIEW_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).PerformanceReviews.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.review_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'PerformanceReviewInitiated',
        payload: { id: created.id, tenantId, review_number: data.review_number },
        tenantId,
      })
    ...
    logger.info('PerformanceReview created', { id, review_number: data.review_number })
    return { id }
  },
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).PerformanceReviews.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('PerformanceReview', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `PerformanceReview '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).PerformanceReviews.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'PerformanceReview',
        entityId: id,
        entityCode: String(before.review_number),
        before,
        after: updated,
      })
    ...
    logger.info('PerformanceReview updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a PerformanceReview.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).PerformanceReviews.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('PerformanceReview soft-deleted', { id })
  },
   ...
   * (e.g. 'AppraisalLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'AppraisalLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'PerformanceReview'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).PerformanceReviews.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('PerformanceReview', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'PerformanceReview' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'PerformanceReview',
        entityId: id,
        entityCode: String(entity.review_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'PerformanceReviewTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('PerformanceReview transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count PerformanceReview records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).PerformanceReviews.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a PerformanceReview with the given review_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(review_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).PerformanceReviews.findFirst({
      where: { tenantId, review_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
