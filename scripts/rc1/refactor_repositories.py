#!/usr/bin/env python3
"""
SUOP ERP — RC1 Fix Pack 1: Repository Refactor
Replaces the generic genRepo (raw SQL) factory in 22 stub-module repositories
with a Prisma-based repository wrapper. The new services already use Prisma
directly (db.<Model>.findMany / create / update / delete), so the repository
file is now a thin compatibility layer that re-exports Prisma-backed helpers
for any consumer that still imports from '../repository'.
"""
...
# (module, primary_model)
MODULES = {
    "accounts-payable": "SupplierInvoices",
    "accounts-receivable": "CustomerInvoices",
    "product-costing": "ProductCosts",
    "general-ledger": "JournalEntries",
    "gst-taxation": "GstConfigurations",
    "employee-master": "Employees",
    "attendance-shift": "Attendance",
    "leave-management": "LeaveApplications",
    "payroll-processing": "PayrollRuns",
    "recruitment-onboarding": "JobPostings",
    "performance-management": "PerformanceReviews",
    "crm-foundation": "CrmActivities",
    "lead-opportunity": "Leads",
    "customer-service": "SupportTickets",
    "complaint-management": "Complaints",
    "after-sales-service": "ServiceRequests",
    "customer-portal": "PortalUsers",
    "bi-foundation": "BiKpiRepository",
    "executive-dashboards": "Dashboards",
    "ai-prediction": "AiPredictions",
    "reporting-platform": "Reports",
    "alerts-kpi-engine": "AlertRules",
}

REPO_TEMPLATE = '''/**
 * __MODULE__ Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on __MODEL__ ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).__MODEL__.findFirst({
  ...
  async list(params: {
    ...
    const [rows, total] = await Promise.all([
      (db as any).__MODEL__.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).__MODEL__.count({ where }),
    ])
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).__MODEL__.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      const existing = await (db as any).__MODEL__.findFirst({
        where: { id, tenantId, deletedAt: null },
        select: { version: true },
      })
      if (!existing) throw new NotFoundError('__MODEL__', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on __MODEL__ '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).__MODEL__.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
  ...
  async count(status?: string) {
    ...
    return (db as any).__MODEL__.count({ where })
  },
}
...
def main():
    ...
    for module, model in MODULES.items():
        ...
        dst = modules_dir / module / "repository" / "index.ts"
        dst.write_text(text)
        print(f"  ✓ Refactored: apps/backend/src/modules/{module}/repository/index.ts")
    print(f"\n✓ Refactored {len(MODULES)} repository files to use Prisma")
