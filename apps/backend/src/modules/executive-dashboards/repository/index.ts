/**
 * executive-dashboards Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on Dashboards ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).Dashboards.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).Dashboards.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('Dashboards', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on Dashboards '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).Dashboards.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
