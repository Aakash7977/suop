/**
 * accounts-receivable Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on CustomerInvoices ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).CustomerInvoices.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).CustomerInvoices.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('CustomerInvoices', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on CustomerInvoices '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).CustomerInvoices.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
