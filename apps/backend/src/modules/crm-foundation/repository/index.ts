/**
 * crm-foundation Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on CrmActivities ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).CrmActivities.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).CrmActivities.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('CrmActivities', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on CrmActivities '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).CrmActivities.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
