/**
 * CrmFoundationService — CrmActivity Service Layer
 *
...
import "@/modules/crm-foundation/workflow"
...
// ─── CrmFoundationService ─────────────────────────────────────────────────────────────

export const CrmFoundationService = {

  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List crm_activities with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { activity_number: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).CrmActivities.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).CrmActivities.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single CrmActivity by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).CrmActivities.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('CrmActivity', id)
    }
    return record
  },
  ...
  // ═══ CREATE ════════════════════════════════════════════════════════════════
  /**
   * Create a new CrmActivity.
   *
   * Business rules:
   *   - activity_number must be unique within tenant
   ...
   *   3. Write event to outbox (event=CrmActivityCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    const { tenantId, userId, userEmail, correlationId } = getContext()

    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.activity_number) {
      throw new ValidationError('CrmActivity.activity_number is required', [
        { field: 'activity_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).CrmActivities.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `CrmActivity with activity_number='${data.activity_number}' already exists`,
        { code: 'CRMACTIVITY_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).CrmActivities.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.activity_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'CrmActivityCreated',
        payload: { id: created.id, tenantId, activity_number: data.activity_number },
        tenantId,
      })
    ...
    logger.info('CrmActivity created', { id, activity_number: data.activity_number })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing CrmActivity.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).CrmActivities.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('CrmActivity', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `CrmActivity '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).CrmActivities.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'CrmActivity',
        entityId: id,
        entityCode: String(before.activity_number),
        before,
        after: updated,
      })
    ...
    logger.info('CrmActivity updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a CrmActivity.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).CrmActivities.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('CrmActivity soft-deleted', { id })
  },
   ...
   * (e.g. 'CrmActivityLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'CrmActivityLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'CrmActivity'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).CrmActivities.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('CrmActivity', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'CrmActivity' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'CrmActivity',
        entityId: id,
        entityCode: String(entity.activity_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'CrmActivityTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
          to: targetState,
          activity_number: entity.activity_number,
        },
        tenantId,
      })
    ...
    logger.info('CrmActivity transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count CrmActivity records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).CrmActivities.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a CrmActivity with the given activity_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(activity_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).CrmActivities.findFirst({
      where: { tenantId, activity_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
