/**
 * gst-taxation Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on GstConfigurations ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).GstConfigurations.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).GstConfigurations.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('GstConfigurations', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on GstConfigurations '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).GstConfigurations.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
