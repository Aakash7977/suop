/**
 * GstTaxationService — GstConfiguration Service Layer
 *
...
export const GstTaxationService = {

  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List gst_configurations with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { config_code: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).GstConfigurations.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).GstConfigurations.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single GstConfiguration by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).GstConfigurations.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('GstConfiguration', id)
    }
    return record
  },
   ...
   *
   * Business rules:
   *   - config_code must be unique within tenant
   *   - status defaults to 'DRAFT' (workflow initialState)
   *   - version initialized to 0
   *
   * Side effects (all in one transaction):
   *   1. INSERT into gst_configurations
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=GstConfigCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.config_code) {
      throw new ValidationError('GstConfiguration.config_code is required', [
        { field: 'config_code', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).GstConfigurations.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `GstConfiguration with config_code='${data.config_code}' already exists`,
        { code: 'GSTCONFIGURATION_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).GstConfigurations.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.config_code),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'GstConfigCreated',
        payload: { id: created.id, tenantId, config_code: data.config_code },
        tenantId,
      })
    ...
    logger.info('GstConfiguration created', { id, config_code: data.config_code })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing GstConfiguration.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).GstConfigurations.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('GstConfiguration', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `GstConfiguration '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).GstConfigurations.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'GstConfiguration',
        entityId: id,
        entityCode: String(before.config_code),
        before,
        after: updated,
      })
    ...
    logger.info('GstConfiguration updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a GstConfiguration.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).GstConfigurations.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('GstConfiguration soft-deleted', { id })
  },
   ...
   * (e.g. 'GstConfigurationLifecycle'), validates the transition is allowed,
 ...
 status: string; version: number }> {
    ...
    const workflowName = 'GstConfigurationLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'GstConfiguration'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).GstConfigurations.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('GstConfiguration', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'GstConfiguration' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'GstConfiguration',
        entityId: id,
        entityCode: String(entity.config_code),
      ...
      await eventBus.writeToOutbox({
        eventName: 'GstConfigurationTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('GstConfiguration transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count GstConfiguration records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).GstConfigurations.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a GstConfiguration with the given config_code exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(config_code: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).GstConfigurations.findFirst({
      where: { tenantId, config_code, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
