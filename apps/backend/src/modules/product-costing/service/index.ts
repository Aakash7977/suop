/**
 * ProductCostingService — ProductCost Service Layer
 *
...
export const ProductCostingService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List product_costs with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { cost_id: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).ProductCosts.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).ProductCosts.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single ProductCost by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).ProductCosts.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('ProductCost', id)
    }
    return record
  },
  ...
  // ═══ CREATE ════════════════════════════════════════════════════════════════
  /**
   * Create a new ProductCost.
   *
   * Business rules:
   *   - cost_id must be unique within tenant
   *   - status defaults to 'DRAFT' (workflow initialState)
   *   - version initialized to 0
   *
   * Side effects (all in one transaction):
   *   1. INSERT into product_costs
   *   2. Write audit log (action=CREATE)
   *   3. Write event to outbox (event=ProductCostCalculated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.cost_id) {
      throw new ValidationError('ProductCost.cost_id is required', [
        { field: 'cost_id', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).ProductCosts.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `ProductCost with cost_id='${data.cost_id}' already exists`,
        { code: 'PRODUCTCOST_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).ProductCosts.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.cost_id),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'ProductCostCalculated',
        payload: { id: created.id, tenantId, cost_id: data.cost_id },
        tenantId,
      })
    ...
    logger.info('ProductCost created', { id, cost_id: data.cost_id })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing ProductCost.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).ProductCosts.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('ProductCost', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `ProductCost '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).ProductCosts.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'ProductCost',
        entityId: id,
        entityCode: String(before.cost_id),
        before,
        after: updated,
      })
    ...
    logger.info('ProductCost updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a ProductCost.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).ProductCosts.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('ProductCost soft-deleted', { id })
  },
   ...
   * (e.g. 'ProductCostLifecycle'), validates the transition is allowed,
 ...
 status: string; version: number }> {
    ...
    const workflowName = 'ProductCostLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'ProductCost'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).ProductCosts.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('ProductCost', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'ProductCost' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'ProductCost',
        entityId: id,
        entityCode: String(entity.cost_id),
      ...
      await eventBus.writeToOutbox({
        eventName: 'ProductCostTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('ProductCost transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count ProductCost records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).ProductCosts.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a ProductCost with the given cost_id exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(cost_id: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).ProductCosts.findFirst({
      where: { tenantId, cost_id, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
