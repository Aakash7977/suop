/**
 * product-costing Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on ProductCosts ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).ProductCosts.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).ProductCosts.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('ProductCosts', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on ProductCosts '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).ProductCosts.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
