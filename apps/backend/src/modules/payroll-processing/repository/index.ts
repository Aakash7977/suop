/**
 * payroll-processing Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on PayrollRuns ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).PayrollRuns.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).PayrollRuns.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('PayrollRuns', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on PayrollRuns '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).PayrollRuns.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
