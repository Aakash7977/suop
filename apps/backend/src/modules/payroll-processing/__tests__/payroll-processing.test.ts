/**
 * payroll-processing Service — Smoke Tests
 *
...
describe('PayrollProcessingService', () => {
  ...
  describe('service shape', () => {
    it('exports an object with required methods', () => {
      expect(PayrollProcessingService).toBeDefined()
      expect(typeof PayrollProcessingService.list).toBe('function')
      expect(typeof PayrollProcessingService.getById).toBe('function')
      expect(typeof PayrollProcessingService.create).toBe('function')
      expect(typeof PayrollProcessingService.update).toBe('function')
      expect(typeof PayrollProcessingService.delete).toBe('function')
      expect(typeof PayrollProcessingService.transition).toBe('function')
      expect(typeof PayrollProcessingService.count).toBe('function')
      expect(typeof PayrollProcessingService.existsByCode).toBe('function')
    })
  })
  ...
  describe('authentication enforcement', () => {
    ...
    it('list() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.list()).rejects.toThrow(AuthorizationError)
    })

    it('getById() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.getById('test-id')).rejects.toThrow(AuthorizationError)
    })

    it('create() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.create({})).rejects.toThrow(AuthorizationError)
    })

    it('update() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.update('id', {})).rejects.toThrow(AuthorizationError)
    })

    it('delete() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.delete('id')).rejects.toThrow(AuthorizationError)
    })

    it('count() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.count()).rejects.toThrow(AuthorizationError)
    })

    it('existsByCode() throws AuthorizationError when no request context', async () => {
      await expect(PayrollProcessingService.existsByCode('code')).rejects.toThrow(AuthorizationError)
    })
  })

  // ═══ Validation ═══════════════════════════════════════════════════════════
  describe('input validation', () => {
    it('create() throws ValidationError when run_number is missing', async () => {
      await _runInTestContext(
        { userId: 'u1', tenantId: 't1', userEmail: 'test@test.com', correlationId: 'c1' },
        async () => {
          await expect(PayrollProcessingService.create({})).rejects.toThrow(ValidationError)
        }
      )
    })
  })
  ...
  describe('workflow registration', () => {
    it('workflow is registered with the registry', async () => {
      ...
      expect(names).toContain('PayrollRunLifecycle')
    })
    ...
    it('transition() throws BusinessRuleError when workflow not found', async () => {
      ...
      await _runInTestContext(
        ...
        async () => {
          ...
          await expect(PayrollProcessingService.transition('nonexistent-id', 'BOGUS_STATE'))
            .rejects.toThrow()
        }
      )
    })
