/**
 * PayrollProcessingService — PayrollRun Service Layer
 *
...
export const PayrollProcessingService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List payroll_runs with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { run_number: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).PayrollRuns.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).PayrollRuns.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single PayrollRun by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).PayrollRuns.findFirst({
   ...
   *
   * Business rules:
   *   - run_number must be unique within tenant
   ...
   *   3. Write event to outbox (event=PayrollRunStarted)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.run_number) {
      throw new ValidationError('PayrollRun.run_number is required', [
        { field: 'run_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).PayrollRuns.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `PayrollRun with run_number='${data.run_number}' already exists`,
        { code: 'PAYROLLRUN_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).PayrollRuns.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.run_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'PayrollRunStarted',
        payload: { id: created.id, tenantId, run_number: data.run_number },
        tenantId,
      })
    ...
    logger.info('PayrollRun created', { id, run_number: data.run_number })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing PayrollRun.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).PayrollRuns.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('PayrollRun', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `PayrollRun '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).PayrollRuns.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'PayrollRun',
        entityId: id,
        entityCode: String(before.run_number),
        before,
        after: updated,
      })
    ...
    logger.info('PayrollRun updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a PayrollRun.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).PayrollRuns.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('PayrollRun soft-deleted', { id })
  },
   ...
   * (e.g. 'PayrollRunLifecycle'), validates the transition is allowed,
 ...
 status: string; version: number }> {
    ...
    const workflowName = 'PayrollRunLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'PayrollRun'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).PayrollRuns.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('PayrollRun', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'PayrollRun' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'PayrollRun',
        entityId: id,
        entityCode: String(entity.run_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'PayrollRunTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('PayrollRun transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count PayrollRun records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).PayrollRuns.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a PayrollRun with the given run_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(run_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).PayrollRuns.findFirst({
      where: { tenantId, run_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
