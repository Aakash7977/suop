/**
 * ReportingPlatformService — Report Service Layer
 *
...
// ─── ReportingPlatformService ─────────────────────────────────────────────────────────────

export const ReportingPlatformService = {

  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List reports with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { report_code: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).Reports.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).Reports.count({ where }),
    ])
  ...
  async getById(id: string): Promise<Record<string, unknown>> {
    ...
    const record = await (db as any).Reports.findFirst({
   ...
   *
   * Business rules:
   *   - report_code must be unique within tenant
   ...
   *   3. Write event to outbox (event=ReportCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.report_code) {
      throw new ValidationError('Report.report_code is required', [
        { field: 'report_code', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).Reports.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `Report with report_code='${data.report_code}' already exists`,
        { code: 'REPORT_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).Reports.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityType: 'Report',
        entityId: created.id,
        entityCode: String(data.report_code),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'ReportCreated',
        payload: { id: created.id, tenantId, report_code: data.report_code },
        tenantId,
      })
    ...
    logger.info('Report created', { id, report_code: data.report_code })
    return { id }
  },
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).Reports.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('Report', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `Report '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).Reports.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'Report',
        entityId: id,
        entityCode: String(before.report_code),
        before,
        after: updated,
      })
    ...
    logger.info('Report updated', { id, version: result.version })
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).Reports.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('Report soft-deleted', { id })
  },
   ...
   * (e.g. 'ReportLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'ReportLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'Report'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).Reports.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('Report', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'Report' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'Report',
        entityId: id,
        entityCode: String(entity.report_code),
      ...
      await eventBus.writeToOutbox({
        eventName: 'ReportTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
          to: targetState,
          report_code: entity.report_code,
        },
        tenantId,
      })
    ...
    logger.info('Report transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count Report records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).Reports.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a Report with the given report_code exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(report_code: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).Reports.findFirst({
      where: { tenantId, report_code, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
