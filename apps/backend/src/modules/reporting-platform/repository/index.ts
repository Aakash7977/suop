/**
 * reporting-platform Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on Reports ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).Reports.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).Reports.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on Reports '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).Reports.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
