/**
 * leave-management Service — Smoke Tests
 *
...
describe('LeaveManagementService', () => {
  ...
  describe('service shape', () => {
    it('exports an object with required methods', () => {
      expect(LeaveManagementService).toBeDefined()
      expect(typeof LeaveManagementService.list).toBe('function')
      expect(typeof LeaveManagementService.getById).toBe('function')
      expect(typeof LeaveManagementService.create).toBe('function')
      expect(typeof LeaveManagementService.update).toBe('function')
      expect(typeof LeaveManagementService.delete).toBe('function')
      expect(typeof LeaveManagementService.transition).toBe('function')
      expect(typeof LeaveManagementService.count).toBe('function')
      expect(typeof LeaveManagementService.existsByCode).toBe('function')
    })
  })
  ...
  describe('authentication enforcement', () => {
    ...
    it('list() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.list()).rejects.toThrow(AuthorizationError)
    })

    it('getById() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.getById('test-id')).rejects.toThrow(AuthorizationError)
    })

    it('create() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.create({})).rejects.toThrow(AuthorizationError)
    })

    it('update() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.update('id', {})).rejects.toThrow(AuthorizationError)
    })

    it('delete() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.delete('id')).rejects.toThrow(AuthorizationError)
    })

    it('count() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.count()).rejects.toThrow(AuthorizationError)
    })

    it('existsByCode() throws AuthorizationError when no request context', async () => {
      await expect(LeaveManagementService.existsByCode('code')).rejects.toThrow(AuthorizationError)
    })
  })

  // ═══ Validation ═══════════════════════════════════════════════════════════
  describe('input validation', () => {
    it('create() throws ValidationError when application_number is missing', async () => {
      await _runInTestContext(
        { userId: 'u1', tenantId: 't1', userEmail: 'test@test.com', correlationId: 'c1' },
        async () => {
          await expect(LeaveManagementService.create({})).rejects.toThrow(ValidationError)
        }
      )
    })
  })
  ...
  describe('workflow registration', () => {
    it('workflow is registered with the registry', async () => {
      ...
      expect(names).toContain('LeaveApplicationLifecycle')
    })
    ...
    it('transition() throws BusinessRuleError when workflow not found', async () => {
      ...
      await _runInTestContext(
        ...
        async () => {
          ...
          await expect(LeaveManagementService.transition('nonexistent-id', 'BOGUS_STATE'))
            .rejects.toThrow()
        }
      )
    })
