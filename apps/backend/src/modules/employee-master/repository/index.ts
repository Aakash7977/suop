/**
 * employee-master Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on Employees ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).Employees.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).Employees.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('Employees', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on Employees '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).Employees.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
