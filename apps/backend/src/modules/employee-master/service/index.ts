/**
 * EmployeeMasterService — Employee Service Layer
 *
...
export const EmployeeMasterService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List employees with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    const [rows, total] = await Promise.all([
      (db as any).Employees.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).Employees.count({ where }),
    ])
  ...
  async getById(id: string): Promise<Record<string, unknown>> {
    ...
    const record = await (db as any).Employees.findFirst({
   ...
   *
   * Business rules:
   *   - employee_code must be unique within tenant
   ...
   *   3. Write event to outbox (event=EmployeeCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    if (!data.employee_code) {
      throw new ValidationError('Employee.employee_code is required', [
        { field: 'employee_code', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).Employees.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `Employee with employee_code='${data.employee_code}' already exists`,
        { code: 'EMPLOYEE_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).Employees.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityType: 'Employee',
        entityId: created.id,
        entityCode: String(data.employee_code),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'EmployeeCreated',
        payload: { id: created.id, tenantId, employee_code: data.employee_code },
        tenantId,
      })
    ...
    logger.info('Employee created', { id, employee_code: data.employee_code })
    return { id }
  },
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).Employees.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('Employee', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `Employee '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).Employees.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'Employee',
        entityId: id,
        entityCode: String(before.employee_code),
        before,
        after: updated,
      })
    ...
    logger.info('Employee updated', { id, version: result.version })
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).Employees.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
   ...
   * (e.g. 'EmployeeLifecycle'), validates the transition is allowed,
  ...
  async transition(id: string, targetState: string, reason?: string): Promise<{ status: string; version: number }> {
    ...
    const workflowName = 'EmployeeLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'Employee'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).Employees.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('Employee', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'Employee' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'Employee',
        entityId: id,
        entityCode: String(entity.employee_code),
      ...
      await eventBus.writeToOutbox({
        eventName: 'EmployeeTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('Employee transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count Employee records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).Employees.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a Employee with the given employee_code exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(employee_code: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).Employees.findFirst({
      where: { tenantId, employee_code, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
