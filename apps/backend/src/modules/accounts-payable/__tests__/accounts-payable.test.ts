/**
 * accounts-payable Service — Smoke Tests
 *
...
describe('AccountsPayableService', () => {
  ...
  describe('service shape', () => {
    it('exports an object with required methods', () => {
      expect(AccountsPayableService).toBeDefined()
      expect(typeof AccountsPayableService.list).toBe('function')
      expect(typeof AccountsPayableService.getById).toBe('function')
      expect(typeof AccountsPayableService.create).toBe('function')
      expect(typeof AccountsPayableService.update).toBe('function')
      expect(typeof AccountsPayableService.delete).toBe('function')
      expect(typeof AccountsPayableService.transition).toBe('function')
      expect(typeof AccountsPayableService.count).toBe('function')
      expect(typeof AccountsPayableService.existsByCode).toBe('function')
    })
  })
  ...
  describe('authentication enforcement', () => {
    ...
    it('list() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.list()).rejects.toThrow(AuthorizationError)
    })

    it('getById() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.getById('test-id')).rejects.toThrow(AuthorizationError)
    })

    it('create() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.create({})).rejects.toThrow(AuthorizationError)
    })

    it('update() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.update('id', {})).rejects.toThrow(AuthorizationError)
    })

    it('delete() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.delete('id')).rejects.toThrow(AuthorizationError)
    })

    it('count() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.count()).rejects.toThrow(AuthorizationError)
    })

    it('existsByCode() throws AuthorizationError when no request context', async () => {
      await expect(AccountsPayableService.existsByCode('code')).rejects.toThrow(AuthorizationError)
    })
  })
  ...
  describe('input validation', () => {
    it('create() throws ValidationError when invoice_number is missing', async () => {
      await _runInTestContext(
        ...
        async () => {
          await expect(AccountsPayableService.create({})).rejects.toThrow(ValidationError)
        }
      )
    })
  })
  ...
  describe('workflow registration', () => {
    it('workflow is registered with the registry', async () => {
      ...
      expect(names).toContain('SupplierInvoiceLifecycle')
    })
    ...
    it('transition() throws BusinessRuleError when workflow not found', async () => {
      ...
      await _runInTestContext(
        ...
        async () => {
          ...
          await expect(AccountsPayableService.transition('nonexistent-id', 'BOGUS_STATE'))
            .rejects.toThrow()
        }
      )
    })
