/**
 * ai-prediction Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on AiPredictions ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).AiPredictions.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).AiPredictions.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('AiPredictions', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on AiPredictions '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).AiPredictions.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
