/**
 * alerts-kpi-engine Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on AlertRules ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).AlertRules.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).AlertRules.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('AlertRules', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on AlertRules '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).AlertRules.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
