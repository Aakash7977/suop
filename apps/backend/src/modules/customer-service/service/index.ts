/**
 * CustomerServiceService — SupportTicket Service Layer
 *
...
export const CustomerServiceService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List support_tickets with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { ticket_number: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).SupportTickets.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).SupportTickets.count({ where }),
    ])
  ...
  // ═══ GET BY ID ═════════════════════════════════════════════════════════════
  /**
   * Fetch a single SupportTicket by ID.
   * Throws NotFoundError if missing, deleted, or belongs to another tenant.
   */
  async getById(id: string): Promise<Record<string, unknown>> {
    const { tenantId } = getContext()

    const record = await (db as any).SupportTickets.findFirst({
   ...
   *
   * Business rules:
   *   - ticket_number must be unique within tenant
   ...
   *   3. Write event to outbox (event=TicketCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    ...
    if (!data.ticket_number) {
      throw new ValidationError('SupportTicket.ticket_number is required', [
        { field: 'ticket_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).SupportTickets.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `SupportTicket with ticket_number='${data.ticket_number}' already exists`,
        { code: 'SUPPORTTICKET_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).SupportTickets.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.ticket_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'TicketCreated',
        payload: { id: created.id, tenantId, ticket_number: data.ticket_number },
        tenantId,
      })
    ...
    logger.info('SupportTicket created', { id, ticket_number: data.ticket_number })
    return { id }
  },
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).SupportTickets.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('SupportTicket', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `SupportTicket '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).SupportTickets.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'SupportTicket',
        entityId: id,
        entityCode: String(before.ticket_number),
        before,
        after: updated,
      })
    ...
    logger.info('SupportTicket updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a SupportTicket.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).SupportTickets.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('SupportTicket soft-deleted', { id })
  },
   ...
   * (e.g. 'TicketLifecycle'), validates the transition is allowed,
 ...
 status: string; version: number }> {
    ...
    const workflowName = 'TicketLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'SupportTicket'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).SupportTickets.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('SupportTicket', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'SupportTicket' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'SupportTicket',
        entityId: id,
        entityCode: String(entity.ticket_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'SupportTicketTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
    ...
    logger.info('SupportTicket transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count SupportTicket records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).SupportTickets.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a SupportTicket with the given ticket_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(ticket_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).SupportTickets.findFirst({
      where: { tenantId, ticket_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
