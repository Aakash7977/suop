/**
 * customer-service Service — Smoke Tests
 *
...
describe('CustomerServiceService', () => {
  ...
  describe('service shape', () => {
    it('exports an object with required methods', () => {
      expect(CustomerServiceService).toBeDefined()
      expect(typeof CustomerServiceService.list).toBe('function')
      expect(typeof CustomerServiceService.getById).toBe('function')
      expect(typeof CustomerServiceService.create).toBe('function')
      expect(typeof CustomerServiceService.update).toBe('function')
      expect(typeof CustomerServiceService.delete).toBe('function')
      expect(typeof CustomerServiceService.transition).toBe('function')
      expect(typeof CustomerServiceService.count).toBe('function')
      expect(typeof CustomerServiceService.existsByCode).toBe('function')
    })
  })
  ...
  describe('authentication enforcement', () => {
    ...
    it('list() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.list()).rejects.toThrow(AuthorizationError)
    })

    it('getById() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.getById('test-id')).rejects.toThrow(AuthorizationError)
    })

    it('create() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.create({})).rejects.toThrow(AuthorizationError)
    })

    it('update() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.update('id', {})).rejects.toThrow(AuthorizationError)
    })

    it('delete() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.delete('id')).rejects.toThrow(AuthorizationError)
    })

    it('count() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.count()).rejects.toThrow(AuthorizationError)
    })

    it('existsByCode() throws AuthorizationError when no request context', async () => {
      await expect(CustomerServiceService.existsByCode('code')).rejects.toThrow(AuthorizationError)
    })
  })

  // ═══ Validation ═══════════════════════════════════════════════════════════
  describe('input validation', () => {
    it('create() throws ValidationError when ticket_number is missing', async () => {
      await _runInTestContext(
        { userId: 'u1', tenantId: 't1', userEmail: 'test@test.com', correlationId: 'c1' },
        async () => {
          await expect(CustomerServiceService.create({})).rejects.toThrow(ValidationError)
        }
      )
    })
  })
  ...
  describe('workflow registration', () => {
    it('workflow is registered with the registry', async () => {
      ...
      expect(names).toContain('TicketLifecycle')
    })
    ...
    it('transition() throws BusinessRuleError when workflow not found', async () => {
      ...
      await _runInTestContext(
        ...
        async () => {
          ...
          await expect(CustomerServiceService.transition('nonexistent-id', 'BOGUS_STATE'))
            .rejects.toThrow()
        }
      )
    })
