/**
 * customer-service Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on SupportTickets ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).SupportTickets.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).SupportTickets.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('SupportTickets', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on SupportTickets '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).SupportTickets.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
