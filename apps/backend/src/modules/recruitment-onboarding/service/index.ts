/**
 * RecruitmentOnboardingService — JobPosting Service Layer
 *
...
export const RecruitmentOnboardingService = {
  ...
  // ═══ LIST ══════════════════════════════════════════════════════════════════
  /**
   * List job_postings with pagination, status filter, and full-text search.
  ...
  async list(params: ListParams = {}): Promise<PaginatedResult<Record<string, unknown>>> {
    ...
    if (params.search) {
      where.OR = [
        { requisition_number: { contains: params.search, mode: 'insensitive' } },
        { description: { contains: params.search, mode: 'insensitive' } },
      ]
    }

    const [rows, total] = await Promise.all([
      (db as any).JobPostings.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      (db as any).JobPostings.count({ where }),
    ])
  ...
  async getById(id: string): Promise<Record<string, unknown>> {
    ...
    const record = await (db as any).JobPostings.findFirst({
      where: { id, tenantId, deletedAt: null },
    })

    if (!record) {
      throw new NotFoundError('JobPosting', id)
    }
    return record
  },
   ...
   *
   * Business rules:
   *   - requisition_number must be unique within tenant
   ...
   *   3. Write event to outbox (event=JobPostingCreated)
   */
  async create(data: CreateInput): Promise<{ id: string }> {
    const { tenantId, userId, userEmail, correlationId } = getContext()

    // ── Validation ──────────────────────────────────────────────────────────
    if (!data.requisition_number) {
      throw new ValidationError('JobPosting.requisition_number is required', [
        { field: 'requisition_number', code: 'REQUIRED', message: 'required' },
      ])
    }

    // ── Business rule: uniqueness within tenant ─────────────────────────────
    const existing = await (db as any).JobPostings.findFirst({
      where: {
        tenantId,
    ...
    if (existing) {
      throw new BusinessRuleError(
        `JobPosting with requisition_number='${data.requisition_number}' already exists`,
        { code: 'JOBPOSTING_DUPLICATE_CODE' }
      )
    }

    // ── Transaction ─────────────────────────────────────────────────────────
    const id = await transaction(async (tx) => {
      const created = await (tx as any).JobPostings.create({
        data: {
          ...data,
          tenantId,
      ...
      await auditService.log({
        ...
        entityId: created.id,
        entityCode: String(data.requisition_number),
        after: created,
      })

      await eventBus.writeToOutbox({
        eventName: 'JobPostingCreated',
        payload: { id: created.id, tenantId, requisition_number: data.requisition_number },
        tenantId,
      })
    ...
    logger.info('JobPosting created', { id, requisition_number: data.requisition_number })
    return { id }
  },

  // ═══ UPDATE ════════════════════════════════════════════════════════════════
  /**
   * Update an existing JobPosting.
   *
   * Business rules:
  ...
  async update(id: string, data: UpdateInput): Promise<{ id: string; version: number }> {
    ...
    const result = await transaction(async (tx) => {
      const before = await (tx as any).JobPostings.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!before) {
        throw new NotFoundError('JobPosting', id)
      ...
      if (expectedVersion !== undefined && before.version !== expectedVersion) {
        throw new ConcurrencyError(
          `JobPosting '${id}' was modified by another transaction (expected version ${expectedVersion}, actual ${before.version})`,
          { entityId: id }
        )
      }

      const updated = await (tx as any).JobPostings.update({
        where: { id },
        data: {
          ...data,
          version: { increment: 1 },
      ...
      await auditService.log({
        ...
        entityType: 'JobPosting',
        entityId: id,
        entityCode: String(before.requisition_number),
        before,
        after: updated,
      })
    ...
    logger.info('JobPosting updated', { id, version: result.version })
    return { id, version: result.version }
  },

  // ═══ DELETE (soft) ═════════════════════════════════════════════════════════
  /**
   * Soft-delete a JobPosting.
  ...
  async delete(id: string, reason?: string): Promise<void> {
    ...
    await transaction(async (tx) => {
      ...
      await (tx as any).JobPostings.update({
        where: { id },
        data: {
          deletedAt: new Date(),
          deletedBy: userId,
    ...
    logger.info('JobPosting soft-deleted', { id })
  },
   ...
   * (e.g. 'CandidateLifecycle'), validates the transition is allowed,
 ...
 status: string; version: number }> {
    ...
    const workflowName = 'CandidateLifecycle'
    let sm: any
    try {
      sm = workflowRegistry.get<any, any>(workflowName)
    } catch {
      throw new BusinessRuleError(
        `Workflow '${workflowName}' not registered for entity 'JobPosting'`,
        { code: 'WORKFLOW.NOT_REGISTERED' }
      )
    }

    const result = await transaction(async (tx) => {
      const entity = await (tx as any).JobPostings.findFirst({
        where: { id, tenantId, deletedAt: null },
      })
      if (!entity) {
        throw new NotFoundError('JobPosting', id)
      ...
      if (!check.allowed) {
        throw new BusinessRuleError(
          `Cannot transition 'JobPosting' from '${entity.status}' to '${targetState}': ${check.reason ?? 'transition not allowed'}`,
      ...
      await auditService.log({
        ...
        entityType: 'JobPosting',
        entityId: id,
        entityCode: String(entity.requisition_number),
      ...
      await eventBus.writeToOutbox({
        eventName: 'JobPostingTransitioned',
        payload: {
          id,
          tenantId,
          from: entity.status,
          to: targetState,
          requisition_number: entity.requisition_number,
        },
        tenantId,
      })
    ...
    logger.info('JobPosting transitioned', {
      id,
      to: targetState,
  ...
  // ═══ COUNT ═════════════════════════════════════════════════════════════════
  /**
   * Count JobPosting records matching optional status filter.
  ...
  async count(params: { status?: string } = {}): Promise<number> {
    ...
    if (params.status) where.status = params.status
    return (db as any).JobPostings.count({ where })
  },

  // ═══ EXISTS ════════════════════════════════════════════════════════════════
  /**
   * Check if a JobPosting with the given requisition_number exists in this tenant.
   * Used by upstream modules to validate references before linking.
   */
  async existsByCode(requisition_number: string): Promise<boolean> {
    const { tenantId } = getContext()
    const r = await (db as any).JobPostings.findFirst({
      where: { tenantId, requisition_number, deletedAt: null },
      select: { id: true },
    })
    return r !== null
  },
