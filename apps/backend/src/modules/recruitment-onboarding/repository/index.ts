/**
 * recruitment-onboarding Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on JobPostings ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).JobPostings.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).JobPostings.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('JobPostings', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on JobPostings '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).JobPostings.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
