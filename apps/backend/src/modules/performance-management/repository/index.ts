/**
 * performance-management Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on PerformanceReviews ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).PerformanceReviews.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).PerformanceReviews.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('PerformanceReviews', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on PerformanceReviews '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).PerformanceReviews.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
