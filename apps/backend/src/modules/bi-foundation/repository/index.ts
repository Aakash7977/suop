/**
 * bi-foundation Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on BiKpiRepository ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).BiKpiRepository.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).BiKpiRepository.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('BiKpiRepository', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on BiKpiRepository '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).BiKpiRepository.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
