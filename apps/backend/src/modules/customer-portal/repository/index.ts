/**
 * customer-portal Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on PortalUsers ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).PortalUsers.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).PortalUsers.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('PortalUsers', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on PortalUsers '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).PortalUsers.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
