/**
 * attendance-shift Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on Attendance ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).Attendance.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).Attendance.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on Attendance '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).Attendance.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
