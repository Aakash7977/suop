/**
 * complaint-management Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on Complaints ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).Complaints.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).Complaints.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('Complaints', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on Complaints '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).Complaints.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
