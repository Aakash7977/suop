/**
 * general-ledger Repository — Prisma-backed
 *
...
// ─── Prisma-backed CRUD operations on JournalEntries ─────────────────────────────
...
export const repository = {
  ...
  async findById(id: string) {
    ...
    return (db as any).JournalEntries.findFirst({
  ...
  async create(data: Record<string, unknown>) {
    ...
    return (db as any).JournalEntries.create({
      data: {
        ...data,
        tenantId,
        version: 0,
  ...
  async update(id: string, data: Record<string, unknown>, expectedVersion?: number) {
    ...
    if (expectedVersion !== undefined) {
      ...
      if (!existing) throw new NotFoundError('JournalEntries', id)
      if (existing.version !== expectedVersion) {
        throw new Error(
          `Concurrency conflict on JournalEntries '${id}': expected version ${expectedVersion}, actual ${existing.version}`
        )
      }
    }

    return (db as any).JournalEntries.update({
      where: { id },
      data: {
        ...data,
        version: { increment: 1 },
